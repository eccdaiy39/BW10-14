...
/**
 * Represents a seventh  extension field element.
 */
typedef fp_t fp7_t[7];

/**
 * Represents a double-precision seventh extension field element.
 */
typedef dv_t dv7_t[7];
...
/**
 * Represents a  14-th extension field element.
 */
typedef fp7_t fp14_t[2];

/**
 * Represents a double-precision 14-th extension field element.
 */
typedef dv7_t dv14_t[2];
...
/**
 * Initializes a double-precision senventh extension field element with a null
 * value.
 *
* @param[out] A			- the senventh extension element to initialize.
 */
#define dv7_null(A)															\
		dv_null(A[0]); dv_null(A[1]); dv_null(A[2]); dv_null(A[3]); dv_null(A[4]); dv_null(A[5]);dv_null(A[6]);							\

/**
 * Allocates a double-precision senventh extension field element.
 *
 * @param[out] A			- the new senventh extension field element.
 */
#define dv7_new(A)															\
		dv_new(A[0]); dv_new(A[1]);dv_new(A[2]);dv_new(A[3]); dv_new(A[4]);dv_new(A[5]);dv_new(A[6]);							\

/**
 * Frees a double-precision senventh extension field element.
 *
 * @param[out] A			- the senventh extension field element to free.
 */
#define dv7_free(A)															\
		dv_free(A[0]); dv_free(A[1]); dv_free(A[2]);dv_free(A[3]); dv_free(A[4]); dv_free(A[5]);dv_free(A[6]);						\

/**
 * Initializes a senventh extension field element with null.
 *
* @param[out] A			- the senventh extension element to initialize.
 */
#define fp7_null(A)															\
		fp_null(A[0]); fp_null(A[1]); fp_null(A[2]);fp_null(A[3]); fp_null(A[4]); fp_null(A[5]);fp_null(A[6]);						\

/**
 * Allocates a senventh extension field element.
 *
 * @param[out] A			- the new senventh extension field element.
 */
#define fp7_new(A)															\
		fp_new(A[0]); fp_new(A[1]);fp_new(A[2]);fp_new(A[3]); fp_new(A[4]);fp_new(A[5]);fp_new(A[6]);							\

/**
 * Frees a senventh extension field element.
 *
 * @param[out] A			- the senventh extension field element to free.
 */
#define fp7_free(A)															\
		fp_free(A[0]); fp_free(A[1]); fp_free(A[2]);fp_free(A[3]); fp_free(A[4]); fp_free(A[5]);fp_free(A[6]);							\

/**
 * Multiplies in seventh extension field elements. Computes
 * C = A * B.
 *
 * @param[out] C			- the result.
 * @param[in] A				- the first seventh extension field element.
 * @param[in] B				- the second seventh extension field element.
 */
#define fp7_mul(C, A, B)	fp7_mul_lazyr(C, A, B)



/**
 * Squaring in seventh extension field elements. Computes
 * C = A^2.
 *
 * @param[out] C			- the result.
 * @param[in] A				- the seventh extension field element.
 */
#define fp7_sqr(C, A)	fp7_sqr_lazyr(C, A)
...
/**
 * Multiplies  two 13-th extension field elements. Computes
 * C = A * B.
 *
 * @param[out] C			- the result.
...
/**
 * Initializes a double-precision 14-th  extension field element with a null
 * value.
 *
* @param[out] A			- the 14-th  extension element to initialize.
 */
#define dv14_null(A)															\
		dv7_null(A[0]); dv7_null(A[1]); 							\

/**
 * Allocates a double-precision 14-th extension field element.
 *
 * @param[out] A			- the new 14-th extension field element.
 */
#define dv14_new(A)															\
		dv7_new(A[0]); dv7_new(A[1]);						\

/**
 * Frees a double-precision 14-th extension field element.
 *
 * @param[out] A			- the 14-th extension field element to free.
 */
#define dv14_free(A)															\
		dv7_free(A[0]); dv7_free(A[1]);						\

/**
 * Initializes a 14-th extension field element with null.
 *
* @param[out] A			- the 14-th extension element to initialize.
 */
#define fp14_null(A)															\
		fp7_null(A[0]); fp7_null(A[1]);						\

/**
 * Allocates a 14-th extension field element.
 *
 * @param[out] A			- the new 14-th extension field element.
 */
#define fp14_new(A)															\
		fp7_new(A[0]); fp7_new(A[1]);						\

/**
 * Frees a 14-th extension field element.
 *
 * @param[out] A			- the 14-th extension field element to free.
 */
#define fp14_free(A)															\
		fp7_free(A[0]); fp7_free(A[1]); 							\

/**
 * Multiplies  two 14-th extension field elements. Computes
 * C = A * B.
 *
 * @param[out] C			- the result.
 * @param[in] A				- the first 14-th extension field element.
 * @param[in] B				- the second 14-th extension field element.
 */
#define fp14_mul(C, A, B)	fp14_mul_lazyr(C, A, B)

/**
 * Squaring  two 14-th extension field elements. Computes
 * C = A * B.
 *
 * @param[out] C			- the result.
 * @param[in] A				- the  14-th extension field element.
 */
#define fp14_sqr(C, A)	fp14_sqr_lazyr(C, A)
...
/**
 * Returns the result of a comparison between two senventh extension field
 * elements.
 *
 * @param[in] a				- the first senventh extension field element.
 * @param[in] b				- the second senventh extension field element.
 * @return RLC_EQ if a == b, and RLC_NE otherwise.
 */
int fp7_cmp(const fp7_t a, const fp7_t b);

/**
 * Returns the result of a signed comparison between an senventh extension field
 * element and a digit.
 *
 * @param[in] a				- the senventh extension field element.
 * @param[in] b				- the digit.
 * @return RLC_EQ if a == b, and RLC_NE otherwise.
 */
int fp7_cmp_dig(const fp7_t a, const dig_t b);

/**
 * Assigns an senventh extension field element to a digit.
 *
 * @param[in] a				- the senventh extension field element.
 * @param[in] b				- the digit.
 */
...
/**
 * Initializes the senventh extension field arithmetic module.
 */
void fp7_field_init(void);

/**
 * Copies the second argument to the first argument.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the seventh extension field element to copy.
 */
void fp7_copy(fp7_t c, const fp7_t a);

/**
 * Assigns zero to a seventh extension field element.
 *
 * @param[out] a			- the seventh extension field element to zero.
 */
void fp7_zero(fp7_t a);

/**
 * Tests if a seventh extension field element is zero or not.
 *
 * @param[in] a				- the seventh extension field element to test.
 * @return 1 if the argument is zero, 0 otherwise.
 */
int fp7_is_zero(const fp7_t a);

/**
 * Assigns a random value to a seventh extension field element.
 *
 * @param[out] a			- the seventh extension field element to assign.
 */
void fp7_rand(fp7_t a);

/**
 * Prints a seventh extension field element to standard output.
 *
 * @param[in] a				- the seventh extension field element to print.
 */
void fp7_print(const fp7_t a);

void fp7_exp(fp7_t c, const fp7_t a, const bn_t b);
int fp7_srt(fp7_t c, const fp7_t a);
int fp7_is_square(const fp7_t a);
/**
 * Adds two seventh extension field elements. Computes c = a + b.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the first seventh extension field element.
 * @param[in] b				- the second seventh extension field element.
...
/**
 * Double two seventh extension field elements. Computes c = 2a.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the seventh extension field element.
 */
...
/**
 * Substract two seventh extension field elements. Computes c = a-b.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the first seventh extension field element.
 * @param[in] b				- the second seventh extension field element.
...
/**
 * half a seventh extension field element. Computes c = a/2.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the  seventh extension field element.
 */
...
/**
 * Negates a seventh extension field element.
 */
void fp7_neg(fp7_t c, const fp7_t a);

/**
 * Reduces a seventh extension element modulo the configured prime p.
 * Computes c = a mod p.
 *
 * @param[out] c			- the result.
...
/**
 * Multiples two seventh extension field elements using lazy reduction.
 *
 * @param[out] c			- the result.
...
/**
 * Squaring a seventh extension field element using lazy reduction.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 13-th extension field element.
 */
...
/**
 * Computes a power of the Frobenius map of a senventh extension field element.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the senventh extension element to conjugate.
 * @param[in] i				- the power of the Frobenius map.
 */
void fp7_frb(fp7_t c, const fp7_t a, int i);

/**
 * Multiplies a senventh extension field element by a power of the constant
 * needed to compute a power of the Frobenius map.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the field element to multiply.
...
/**
 * Inverts a senventh extension field element. Computes c = 1/a.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the senventh extension field element to invert.
 */
void fp7_inv(fp7_t c, const fp7_t a);

/**
 * Inverts senventh octic extension field elements simultaneously.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the senventh extension field elements to invert.
 * @param[in] n				- the number of elements.
 */
void fp7_inv_sim(fp7_t * c, const fp7_t * a, int n);
...
/**
 * Returns the result of a comparison between two 14-th extension field
 * elements.
 *
 * @param[in] a				- the first 14-th extension field element.
 * @param[in] b				- the second 14-th extension field element.
 * @return RLC_EQ if a == b, and RLC_NE otherwise.
 */
int fp14_cmp(const fp14_t a, const fp14_t b);

/**
 * Returns the result of a signed comparison between an 14-th extension field
 * element and a digit.
 *
 * @param[in] a				- the 14-th extension field element.
 * @param[in] b				- the digit.
 * @return RLC_EQ if a == b, and RLC_NE otherwise.
 */
int fp14_cmp_dig(const fp14_t a, const dig_t b);

/**
 * Assigns an 14-th extension field element to a digit.
 *
 * @param[in] a				- the 14-th extension field element.
 * @param[in] b				- the digit.
 */
...
/**
 * Initializes the 14-th extension field arithmetic module.
 */
void fp14_field_init(void);

/**
 * Copies the second argument to the first argument.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 14-th extension field element to copy.
 */
void fp14_copy(fp14_t c, const fp14_t a);

/**
 * Assigns zero to a 14-th extension field element.
 *
 * @param[out] a			- the 14-th extension field element to zero.
 */
void fp14_zero(fp14_t a);

/**
 * Tests if a 14-th extension field element is zero or not.
 *
 * @param[in] a				- the 14-th extension field element to test.
 * @return 1 if the argument is zero, 0 otherwise.
 */
int fp14_is_zero(const fp14_t a);

/**
 * Assigns a random value to a 14-th extension field element.
 *
 * @param[out] a			- the 14-th extension field element to assign.
 */
void fp14_rand(fp14_t a);

/**
 * Prints a 14-th extension field element to standard output.
 *
 * @param[in] a				- the 14-th extension field element to print.
 */
void fp14_print(const fp14_t a);


/**
 * Adds two 14-th extension field elements. Computes c = a + b.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the first 14-th extension field element.
 * @param[in] b				- the second 14-th extension field element.
...
/**
 * Double two 14-th extension field elements. Computes c = 2a.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 14-th extension field element.
 */
...
/**
 * Substract two 14-th extension field elements. Computes c = a-b.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the first 14-th extension field element.
 * @param[in] b				- the second 14-th extension field element.
...
/**
 * half a 14-th extension field element. Computes c = a/2.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the  14-th extension field element.
 */
...
/**
 * Negates a 14-th extension field element.
 */
void fp14_neg(fp14_t c, const fp14_t a);

/**
 * Reduces a 14-th extension element modulo the configured prime p.
 * Computes c = a mod p.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the digit vector to reduce.
...
/**
 * Multiples two 14-th extension field elements using lazy reduction.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 14-th extension field element.
 * @param[in] b				- the 14-th extension field element.
 */
void fp14_mul_lazyr(fp14_t d, const fp14_t a, const fp14_t b);

/**
 * Squaring a 14-th extension field element using lazy reduction.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 14-th extension field element.
 */
...
/**
 * Computes a power of the Frobenius map of a 14-th extension field element.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 14-th extension element to conjugate.
 * @param[in] i				- the power of the Frobenius map.
 */
void fp14_frb(fp14_t c, const fp14_t a, int i);

/**
 * Multiplies a 14-th extension field element by a power of the constant
 * needed to compute a power of the Frobenius map.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the field element to multiply.
 ...
 * @param[in] j				- the power of the constant.
 */
void fp14_mul_frb(fp14_t c, const fp14_t a,int i);


/**
 * Inverts a 14-th extension field element. Computes c = 1/a.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 14-th extension field element to invert.
 */
void fp14_inv(fp14_t c, const fp14_t a);

/**
 * Computes the inverse of a cyclotomic octdecic extension field element.
 * For unitary elements, this is equivalent to computing the conjugate.
 * A unitary element is one previously raised to the (p^7 - 1)-th power.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 14-th extension field element to invert.
 */
void fp14_inv_cyc(fp14_t c, const fp14_t a);
...
void fp14_exp_cyc(fp14_t c, const fp14_t a, const bn_t b);
#endif /* !RLC_FPX_H */
