...
void bn_rec_frb_gt_bw14(bn_t* ki, const bn_t k, const bn_t x, const bn_t n) {
	int i, l;
	bn_t u[10];bn_t y[6];
	RLC_TRY {
		...
		bn_sqr(u[0], x);
		bn_copy(u[9], u[0]);
		for (i = 1; i <9; i++)bn_mul(u[i], u[i-1], x);
		...
		bn_mul(ki[0], u[4], x);
		bn_sub(ki[0], ki[0], u[5]);
		bn_mul(ki[0], ki[0], x);
		bn_sub(ki[0], ki[0], u[5]);
		bn_add(ki[0], ki[0], u[0]);
		bn_sub(ki[0], k, ki[0]);

		

		bn_mul(ki[1], u[2], x);
		bn_add(ki[1], ki[1], u[4]);
		bn_add(ki[1], ki[1], u[5]);
		bn_mul(ki[1], ki[1], x);
		bn_sub(ki[1], u[0], ki[1]);

		bn_mul(ki[2], u[3], x);
		bn_add(ki[2], ki[2], u[2]);
		bn_sub(ki[2], u[5], ki[2]);
		bn_mul(ki[2], ki[2], x);
		bn_sub(ki[2], ki[2], u[0]);	
		bn_sub(ki[2], ki[2], u[4]);

		bn_mul(ki[3], u[1], x);
		bn_add(ki[3], ki[3], u[3]);
		bn_add(ki[3], ki[3], u[5]);
		bn_mul(ki[3], ki[3], x);
		bn_sub(ki[3], u[0], ki[3]);
		bn_sub(ki[3], ki[3], u[2]);
		...
		bn_mul(ki[4], u[0], x);
		bn_add(ki[4], ki[4], u[5]);
		bn_sub(ki[4], ki[4], u[1]);
		bn_mul(ki[4], ki[4], x);
		bn_sub(ki[4], ki[4], u[3]);
		bn_sub(ki[4], ki[4], u[0]);

		bn_mul(u[5], u[5], x);
		bn_sub(u[5], u[0], u[5]);
		bn_add_dig(ki[5], x, 1);
		bn_mul(ki[5], ki[5], u[5]);
		bn_sub(ki[5], ki[5], u[1]);
		
		for(i = 0; i<6; i++) ki[i]->used = 1;
	}RLC_CATCH_ANY {
		RLC_THROW(ERR_CAUGHT);
	} RLC_FINALLY {
		for (i = 0; i < 6; i++)bn_free(y[i]);
		for (i = 0; i < 10; i++)bn_free(u[i]);
	}
