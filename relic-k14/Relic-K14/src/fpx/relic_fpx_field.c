...
void fp7_field_init(void) {
	...
	RLC_TRY {
		...
		/* Compute （a）^(p - 1)/7 and consecutive powers. */
		#if FP_PRIME == 340
			fp_set_dig(t0, 4);
			fp_neg(t0, t0);	
		#elif FP_PRIME == 351
		...
		bn_div_dig(e, e, 7);
		fp_exp(t1, t0, e);
		fp_copy(ctx->frb7_1[1],t1);
		for(int i =2; i <7; i++)fp_mul(ctx->frb7_1[i],ctx->frb7_1[i-1],ctx->frb7_1[1]);
		for(int i =1; i <7; i++){
			fp_sqr(ctx->frb7_2[i],ctx->frb7_1[i]);	
			fp_mul(ctx->frb7_3[i],ctx->frb7_2[i],ctx->frb7_1[i]);	
			fp_mul(ctx->frb7_4[i],ctx->frb7_3[i],ctx->frb7_1[i]);
			fp_mul(ctx->frb7_5[i],ctx->frb7_4[i],ctx->frb7_1[i]);
			fp_mul(ctx->frb7_6[i],ctx->frb7_5[i],ctx->frb7_1[i]);														
		}
		bn_div_dig(e, e, 2);		
		fp_exp(t1, t0, e);		
		fp_copy(ctx->frb7_1b[0],t1);
		for(int i =1; i <7; i++)fp_mul(ctx->frb7_1b[i],ctx->frb7_1[i],ctx->frb7_1b[0]);
		for(int i =0; i <7; i++)
		{
		fp_mul(ctx->frb7_2b[i],ctx->frb7_1b[i],ctx->frb7_1b[i]);	
		fp_mul(ctx->frb7_3b[i],ctx->frb7_2b[i],ctx->frb7_1b[i]);	
		fp_mul(ctx->frb7_4b[i],ctx->frb7_3b[i],ctx->frb7_1b[i]);	
		fp_mul(ctx->frb7_5b[i],ctx->frb7_4b[i],ctx->frb7_1b[i]);
		fp_mul(ctx->frb7_6b[i],ctx->frb7_5b[i],ctx->frb7_1b[i]);
		fp_mul(ctx->frb7_7b[i],ctx->frb7_6b[i],ctx->frb7_1b[i]);
		}
