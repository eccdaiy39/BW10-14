...
int fp7_cmp(const fp7_t a, const fp7_t b)
{
	return (fp_cmp(a[0], b[0]) == RLC_EQ) && (fp_cmp(a[1], b[1]) == RLC_EQ) && (fp_cmp(a[2], b[2]) == RLC_EQ) && (fp_cmp(a[3], b[3]) == RLC_EQ) 
		&& (fp_cmp(a[4], b[4]) == RLC_EQ) && (fp_cmp(a[5], b[5]) == RLC_EQ) && (fp_cmp(a[6], b[6]) == RLC_EQ) ? RLC_EQ : RLC_NE;
}

int fp7_cmp_dig(const fp7_t a, dig_t b) {
	return (fp_cmp_dig(a[0], b) == RLC_EQ) && fp_is_zero(a[1]) ?
			RLC_EQ : RLC_NE && fp_is_zero(a[2]) ?
			RLC_EQ : RLC_NE && fp_is_zero(a[3]) ?
			RLC_EQ : RLC_NE && fp_is_zero(a[4]) ?
			RLC_EQ : RLC_NE && fp_is_zero(a[5]) ?
			RLC_EQ : RLC_NE && fp_is_zero(a[6]) ?
			RLC_EQ : RLC_NE;
}
...
int fp13_cmp(const fp13_t a, const fp13_t b)
{
	return (fp_cmp(a[0], b[0]) == RLC_EQ) && (fp_cmp(a[1], b[1]) == RLC_EQ) && (fp_cmp(a[2], b[2]) == RLC_EQ) && (fp_cmp(a[3], b[3]) == RLC_EQ) 
		...
		&& (fp_cmp(a[8], b[8]) == RLC_EQ) && (fp_cmp(a[9], b[9]) == RLC_EQ) && (fp_cmp(a[10], b[10]) == RLC_EQ) && (fp_cmp(a[11], b[11]) == RLC_EQ)
		&& (fp_cmp(a[12], b[12]) == RLC_EQ) ? RLC_EQ : RLC_NE;
}
int fp13_cmp_dig(const fp13_t a, dig_t b) {
	return (fp_cmp_dig(a[0], b) == RLC_EQ) && fp_is_zero(a[1]) ?
			...
			RLC_EQ : RLC_NE && fp_is_zero(a[5]) ?
			RLC_EQ : RLC_NE && fp_is_zero(a[6]) ?
			RLC_EQ : RLC_NE && fp_is_zero(a[7]) ?
			RLC_EQ : RLC_NE && fp_is_zero(a[8]) ?
			RLC_EQ : RLC_NE && fp_is_zero(a[9]) ?
			RLC_EQ : RLC_NE && fp_is_zero(a[10]) ?
			RLC_EQ : RLC_NE && fp_is_zero(a[11]) ?
			RLC_EQ : RLC_NE && fp_is_zero(a[12]) ?
			RLC_EQ : RLC_NE;
}


int fp14_cmp(const fp14_t a, const fp14_t b) {
	return (fp7_cmp(a[0], b[0]) == RLC_EQ) && (fp7_cmp(a[1], b[1]) == RLC_EQ) ?
			RLC_EQ : RLC_NE;
}

int fp14_cmp_dig(const fp14_t a, const dig_t b) {
	return (fp7_cmp_dig(a[0], b) == RLC_EQ) && fp7_is_zero(a[1]) ?
			RLC_EQ : RLC_NE;
}
