...
void fp7_inv(fp7_t c, const fp7_t a){
	...
	RLC_TRY{
		...
		fp7_mul(t0, t0, t1);//a^p*a^(p^2)*...*a^(p^6)

		fp_muln_low(u0, t0[1], a[6]);
		for(int i = 2; i<7; i++){
			fp_muln_low(u1, t0[i], a[7-i]);
			fp_addd_low(u0, u0, u1);		
		}
			...
			fp_addd_low(u0, u0, u0);
			fp_muln_low(u1, t0[0], a[0]);
			fp_addd_low(u0, u1, u0);
		#endif

		fp_rdc(t, u0);
		fp_inv(t, t);
		for(int i = 0; i<7; i++)fp_mul(c[i], t, t0[i]);
	}
	RLC_CATCH_ANY {
		RLC_THROW(ERR_CAUGHT);
	}
